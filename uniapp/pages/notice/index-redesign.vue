<template>
  <view class="container">
    ...
    >
      <view class="detail-popup" v-if="currentNotice">
        <view class="detail-header">
          <view class="detail-tag" :style="{backgroundColor: currentNotice.tagBgColor}">{{ currentNotice.type }}</view>
          <text class="detail-title">{{ currentNotice.title }}</text>
          <u-icon name="close" size="24" color="#909399" @click="showDetail = false" class="close-icon"></u-icon>
        </view>
        <u-line color="#f3f4f6" margin="20rpx 0"></u-line>
        <view class="detail-info">
          <view class="info-item">
            <u-icon name="calendar" size="16" color="#909399"></u-icon>
            <text class="info-text">{{ currentNotice.time }}</text>
          </view>
          <view class="info-item">
            <u-icon name="account" size="16" color="#909399"></u-icon>
            <text class="info-text">{{ currentNotice.publisher }}</text>
          </view>
        </view>
        <scroll-view scroll-y class="detail-content-scroll">
          <view class="detail-content">
            <text class="content-text">{{ currentNotice.fullContent || currentNotice.content }}</text>
          </view>
        </scroll-view>
        <view class="detail-footer">
          <u-button 
            ...
            :customStyle="{background: 'linear-gradient(135deg, #4a7aff 0%, #1d4ed8 100%)', boxShadow: '0 4rpx 12rpx rgba(0, 0, 0, 0.15)', border: 'none'}"
          ></u-button>
        </view>
      </view>
    </u-popup>
