<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:96:"/www/wwwroot/cuijingshui.zwzdoac.cn/public/../application/admin/view/shop/reneworder/detail.html";i:1710987797;s:78:"/www/wwwroot/cuijingshui.zwzdoac.cn/application/admin/view/layout/default.html";i:1710226298;s:75:"/www/wwwroot/cuijingshui.zwzdoac.cn/application/admin/view/common/meta.html";i:1710223706;s:90:"/www/wwwroot/cuijingshui.zwzdoac.cn/application/admin/view/shop/reneworder/invoicetpl.html";i:1709960820;s:77:"/www/wwwroot/cuijingshui.zwzdoac.cn/application/admin/view/common/script.html";i:1710223708;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
            ...
            <div class="row">
                        ...
                        <li class="list-group-item order-status">
                            <div>
                                ...
                                <?php if($row['orderstate']==0): if($row['paystate'] == 0): ?>
                                    ...
                                    <?php elseif($row['paystate'] == 1): if($row['shippingstate'] == 0): if($row['install_status'] == 1): ?>
                                            <span class="label label-danger">待确认</span>
                                            <?php elseif($row['install_status'] == 2): ?>
                                            <span class="label label-danger">待安装</span>
                                             <?php endif; elseif($row['shippingstate'] == 1): ?>
                            ...
                            <div>
                                            <?php if($row['orderstate']==0 || $row['orderstate']==4): if($row['paystate'] == 0): ?>
                                                ...
                                                <?php elseif($row['paystate'] == 1): if($row['shippingstate'] == 0): if($row['install_status'] == 1): ?>
                                           <button type="button" class="btn btn-danger btn-status-click" data-order='{"install_status":2}'>确认完成</button>
                ...
                <div class="col-xs-6">
                    <ul class="list-group">
                        ...
                        <li class="list-group-item">支付时间：<?php if(!(empty($row['paytime']) || (($row['paytime'] instanceof \think\Collection || $row['paytime'] instanceof \think\Paginator ) && $row['paytime']->isEmpty()))): ?><?php echo date("Y-m-d
                            H:i:s",$row['paytime']); endif; ?>
                        </li>
                        <li class="list-group-item">订单类型：续费订单
                        </li>
                        <li class="list-group-item">续费状态：
                            <?php if($row['order_renew_status']==10): ?>待续约
                            <?php elseif($row['order_renew_status']==20): ?>续约成功
                            <?php else: ?>已到期
                            <?php endif; ?>
                        </li>
                        <li class="list-group-item">到期时间：<?php echo $row['order_renew_time_text']; ?></li>
                        <li class="list-group-item">是否退款：

                            <?php if($row['renew_refund_status']==1): ?>未退款
                            <?php elseif($row['renew_refund_status']==2): ?>已退款 <?php echo date("Y-m-d
                            H:i:s",$row['renew_refund_time']); endif; ?>
                        </li>
                        ...
                        <li class="list-group-item">
                            ...
                            <div class="memo-operate">
                                ...
                                <a class="btn btn-success hide btn-cancel" data-status="cancel"
                                   href="javascript:;">取消</a>
                                <a class="btn btn-success hide btn-save" data-status="save" data-field="memo"
                                   data-id="<?php echo $row['id']; ?>" href="javascript:;">保存</a>
                            </div>
                        </li>
                    </ul>
                </div>
                        ...
                        <td class="text-center">
                            <div style="display: flex;justify-content: center;align-items: center;">
                                ...
                                <div style="padding-left: 10px;">
                                    <a class="btn btn-success btn-dialog <?php echo $auth->check('shop/order_aftersales/index')?'':'hide'; ?>"
                                       href="javascript:;"
                                       data-url="shop/order_aftersales/edit/order_goods_id/<?php echo $item['id']; ?>" title="审核售后">审核售后</a>
                                </div>
                                <?php endif; ?>
                            </div>
                        </td>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <table class="table table-bordered table-goods">
            ...
            <tbody>
            <?php if(is_array($row['orderRenew']) || $row['orderRenew'] instanceof \think\Collection || $row['orderRenew'] instanceof \think\Paginator): if( count($row['orderRenew'])==0 ) : echo "" ;else: foreach($row['orderRenew'] as $key=>$item): ?>
            <tr>
                <th scope="row"><?php echo $key+1; ?></th>
                <td><?php echo $item['year']; ?></td>
                <td><?php echo $item['price']; ?></td>
                <td><?php echo $item['pay_time']; ?></td>
                <td><?php echo $item['become_time']; ?></td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            </tbody>
        </table>
    ...
    <div style="padding:15px 15px 0 15px;">
        <ul class="list-group">
            <li class="list-group-item shipper">
                ...
                <input class="form-control selectpage" data-primary-key="name" data-source="shop/shipper/index"
                       style="flex:1;" id="expressname" value="<%=expressname%>"/>
            </li>
            <li class="list-group-item">
