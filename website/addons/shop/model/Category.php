...
class Category Extends Model
{
    ...
    public function getHasChildAttr($value, $data)
    {
        ...
        if (is_null(self::$parentIds)) {
            self::$parentIds = self::where('pid', '>', 0)->cache(true)->where('status', 'normal')->column('pid');
        }
    ...
    public function getHasNavChildAttr($value, $data)
    {
        ...
        if (is_null(self::$navParentIds)) {
            self::$navParentIds = self::where('pid', '>', 0)->cache(true)->where('status', 'normal')->where('isnav', 1)->column('pid');
        }
    ...
    public static function getCategoryList($tag)
    {
        ...
        if ($paginate) {
            ...
            $list = $CategoryModel->paginate($listRows, (isset($paginateArr[1]) ? $paginateArr[1] : false), $config);
        } else {
            $list = $CategoryModel->limit($limit)->cache($cache)->select();
        }
