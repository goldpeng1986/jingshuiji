	...
	<view class="">
		...
		<view class="login">
			...
			<view class="u-p-t-30 u-text-center u-flex">
				...
				<text class="u-font-30 agree" @click="goPage('/pages/page/page?diyname=agreement')" :style="[{ color: theme.bgColor }]">《用户协议》</text>
			</view>
		...
		getPhoneNumber(e) {
			...
			uni.login({
				...
				success: function(loginRes) {
					...
					let data = { encryptedData: e.detail.encryptedData, iv: e.detail.iv, code: loginRes.code };
					if (that.vuex_invite_id) {
						data.invite_id = that.vuex_invite_id;
					}
					that.$api.getWechatMobile(data).then(function(res) {
						if (res.code == 1) {
