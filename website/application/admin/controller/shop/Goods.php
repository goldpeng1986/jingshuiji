...
class Goods extends Backend
{
    ...
    protected function addGoodsRenew($renew,$goods_id)
    {
        $data = [];
       ...
       $result =  (new GoodsRenew())->saveAll($data);
       if(!$result){
           throw new Exception('续费金额不对');
       }
    }
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    //添加预约订单续费
                    if (isset($params['renew'])&&$params['goods_type']==20) {
                        //整理数据
                        $renew = json_decode($params['renew'],true);
                        $this->addGoodsRenew($renew,$this->model->id);
                    }
    ...
    public function edit($ids = null)
    {
            ...
            if ($params) {
                ...
                try {
                    ...
                    if (isset($params['renew']) && $params['goods_type']==20) {
                        //整理数据
                        $renew = json_decode($params['renew'],true);
                        $this->editGoodsRenew($renew,$row->id);
                    }
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
        ...
        //获取续费价格
        $renew = new GoodsRenew();
        $renewinfo = $renew->where('goods_id',$row->id)->where('switch',1)->select();
        $renewlist = [];
        foreach ($renewinfo as $key=>$value){
            $renewlist [$value->renew_year] = $value->renew_price;
        }
        $this->view->assign("renew",$renewlist);
        return $this->view->fetch();
    }

    protected function editGoodsRenew($renew,$goods_id){
        $renewModel = new GoodsRenew();
         $renewModel->where('goods_id',$goods_id)->delete();
        ...
        $result =  $renewModel->saveAll($data);
        if(!$result){
            throw new Exception('续费金额不对');
        }
    }
