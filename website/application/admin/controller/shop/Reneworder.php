 ...
 * @icon fa fa-circle-o
 */
class Reneworder extends Backend
{
     ...
     * 退款
     */
    public  function refundRenew($order_sn, $paytype, $payamount)
    {
    ...
    public function edit_status()
    {
        ...
        $install_status = $this->request->post('install_status');
        ...
        if ($orderstate == 3 && $order->orderstate == 0 && $order->paystate == 1) {
            $this->refundRenew($order->order_sn,$order->paytype,$order->payamount);
            $order->orderstate = 3;
            $order->renew_refund_status	 = 2;
            $order->renew_refund_time = time();
            $order->save();
            TemplateMsg::sendTempMsg('renew_order','order_renew_remind', $order->order_sn);
            //进行退款
            OrderAction::push($order->order_sn, '订单已安装完成', '管理员');
            //记录操作
            OrderAction::push($order->order_sn, '订单押金已退还', '管理员');
            $this->success('操作成功');

        } elseif ($order->orderstate == 4) {
            $this->error("请完成售后后再进行操作");
        }elseif($install_status==2){
            $order->install_status = 2;
            $order->save();
            //发送订阅消息
            TemplateMsg::sendTempMsg('renew_order','order_renew_wait', $order->order_sn);
            OrderAction::push($order->order_sn, '订单已确认', '管理员');
            $this->success('操作成功');
        }
        $this->error('没有权限操作');
    }
