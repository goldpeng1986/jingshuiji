...
class Order extends Model
{
        ...
        self::afterWrite(function ($row) {
            ...
            if (isset($changeData['shippingstate'])) {
                switch ($changeData['shippingstate']) {
                    case 1:
                        //发送通知
                        TemplateMsg::sendTempMsg('goods_order','order_deliver', $row->order_sn);
                        $memo = '订单发货成功';
                        break;
                    case 2:
                        $memo = '更改订单已收货';
    ...
    //获取新订单未推送的数量
    public function getTaskNewOrder(){
        return $this->where('news_remind',1)
            ->where('orderstate',0)
            ->where('paystate',1)
            ->find();
    }
    //获取订单快过期提醒
    public function getTaskRenewOrder(){
        return $this
            ->where('order_renew_status',20)
            ->where('order_type',20)
            ->where('orderstate',3)
            ->where('paystate',1)
            ->where('install_status_remind',1)
