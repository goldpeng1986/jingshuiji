...
class Reneworder extends Model
{
    ...
    public function getOrderRenewTimeTextAttr($value, $data)
    {
        $value = $value ? $value : (isset($data['order_renew_time']) ? $data['order_renew_time'] : '');
        if($value==0){
            return '未续约';
        }
    ...
    public static function refund($order_sn, $paytype, $payamount)
    {
        ...
        try {
            $order = Reneworder::getByOrderSn($order_sn);
            if ($paytype == 'wechat') {
    ...
    {
        return $this->hasMany('OrderRenew', 'order_id', 'id');
    }
}
