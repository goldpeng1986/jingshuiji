define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'shop/reneworder/index' + location.search,
                    // add_url: 'shop/order/add',
                    // edit_url: 'shop/order/edit',
                    del_url: 'shop/reneworder/del',
                    multi_url: 'shop/reneworder/multi',
                    import_url: 'shop/reneworder/import',
                    table: 'shop_order',
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [{
                        ...
                        {
                            field: 'renew_refund_status',
                            title: __('是否退款'),
                            formatter: Controller.api.formatter.renew_refund_status_text
                        },
                        {
        ...
        detail: function () {
            ...
            $(document).on('click', '.btn-status-click', function () {
                ...
                $.post('shop/reneworder/edit_status', {
                    order_id,
                    ...order
                }, function (res) {
                    if (res.code == 1) {
        ...
        api: {
            ...
            formatter:{
                order_renew_status_text(value){
                   if(value==10){
                       return '<a><span class="">待续约</span></a>';
                   }
                   if(value==20){
                       return '<a><span class="text-success">正常</span></a>';
                   }
                    if(value==30){
                        return '<a><span class="text-danger">已到期</span></a>';
                    }
                },
                ...
                renew_refund_status_text(value){
                    if(value==1){
                        return '<a><span class="text-info">未退款</span></a>';
                    }
                    if(value==2){
                        return '<a><span class="text-danger">已退款</span></a>';
                    }
                },
